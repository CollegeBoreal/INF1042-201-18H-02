...
trait RochePapierCiseaux extends GameContext {

  override val moves = List(Roche, Papier, Ciseaux)

  override protected val beats: Map[Move, List[Move]] =
    Map(Roche -> List(Ciseaux), Ciseaux -> List(Papier, Roche), Papier -> List(Roche, Ciseaux))
