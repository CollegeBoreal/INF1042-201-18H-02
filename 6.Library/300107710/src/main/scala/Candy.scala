...
object Candy {
  ...
  def main(args: Array[String]): Unit ={
    ...
    assert(simulateMachine(List(Turn)).run(Machine(locked = true, 1, 0))._1 == Machine(true, 1, 0) )
    assert(simulateMachine(List(Coin, Turn)).run(Machine(locked = true, 1, 0))._1 == Machine(true, 0,1))
    assert(simulateMachine(List(Coin, Turn, Coin)).run(Machine(locked = true, 1, 0))._1== Machine(true ,0, 1))
  }
